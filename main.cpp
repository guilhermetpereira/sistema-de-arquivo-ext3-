    ...
    int main(int argc, char const *argv[])
    {
    	...
    	//	delete[] buffer;
    		...
    		if(parent_dir->SIZE % T == 0 && parent_dir->SIZE > 0)
    		{
    			...
    			if(avaible_blocks_dir[0] == 0)
    			{
    				cout << "No space for this file on blocks!" << endl;
    				return -1;
    			}
    			/* att bitmap but doesnt write to file system yet, necessary to check if directorie will have space for the new file*/ 
    			bit_map[(int)avaible_blocks_dir[0]/8] |= 1 << avaible_blocks_dir[0];
    		...
    		{	
    			...
    			*file_inode = INIT_ROOT;
    	        ...
    	        int string_size = 0;// content.size()-1;
    			
    			for (int i = 0; i < blocks_needed; i++)
    			{
    				file.seekp((avaible_blocks_file[i]-N)*T  ,ios_base::end);
    				for (int i = 0; i < T; ++i)
    				{
    					if(string_size < content.size())
    					{
